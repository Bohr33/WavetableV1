...
//-----------------------------------------------//
//-----Standard Wavetable Oscailltor Model-------//
//-----------------------------------------------//
WavetableOscilatorVoice::WavetableOscilatorVoice(std::shared_ptr<const Wavetable> wavetable, int tableSize) : m_tableSize(tableSize), m_wavetable(wavetable)
{};

bool WavetableOscilatorVoice::canPlaySound(juce::SynthesiserSound*)
{return true;};

void WavetableOscilatorVoice::startNote(int midiNoteNumber, float velocity, juce::SynthesiserSound *sound, int currentPitchWheelPosition)
{
    //Zero previous values
    m_angle = 0.0;
    m_angleDelta = 0.0;
...
void WavetableOscilatorVoice::stopNote(float velocity, bool allowTailOff)
{
    envelope.noteOff();
};

void WavetableOscilatorVoice::prepare(double sampleRate)
{
    envelope.setSampleRate(sampleRate);
}


void WavetableOscilatorVoice::renderNextBlock(juce::AudioBuffer<float> &outputBuffer, int startSample, int numSamples)
{
    ...
    //Change From Here ------//
    std::shared_ptr<const Wavetable> wavetable = std::atomic_load(&m_wavetable);
...
//Main table reading function, performs basic linearly interpolation expacting a guard point
float WavetableOscilatorVoice::interpNextSamp(std::vector<float>& table)
{
    jassert(table.size() == 2049 || table.size() == 2048);
...
void WavetableOscilatorVoice::setFrequency(float frequency)
{
    m_currentFreq = frequency;
    float cyclesPerSample = frequency / getSampleRate();
    m_angleDelta = cyclesPerSample * (float) m_tableSize;
...
void WavetableOscilatorVoice::updateAngle()
{
    m_angle += m_angleDelta;
    if(m_angle >= m_tableSize)
        m_angle -= m_tableSize;
...
float WavetableOscilatorVoice::interpolate(float interp_val, float val1, float val2)
{
    auto interpVal = juce::jlimit(0.0f, 1.0f, interp_val);
...
//Setter Function for Audio Processor Value True State
void WavetableOscilatorVoice::setAPVTS(juce::AudioProcessorValueTreeState* apvts)
{
    apvtsRef = apvts;
...
void WavetableOscilatorVoice::setWavetable(std::shared_ptr<const Wavetable> newWavetable)
{
    std::atomic_store(&m_wavetable, newWavetable);
}



//void WavetableOscilatorVoice::setVoiceOneMipMap(std::shared_ptr<const MipMap> newMipMap)
//{
//    std::atomic_store(&m_mipmapA, newMipMap);
//}
//
//void WavetableOscilatorVoice::setVoiceTwoMipMap(std::shared_ptr<const MipMap> newMipMap)
//{
//    std::atomic_store(&m_mipmapB, newMipMap);
//}
...
void WavetableOscilatorVoice::reportMipMaps()
{
    auto tableSize1 = m_mipmapA->size();
...
float WavetableOscilatorVoice::calculateBendFreq()
{
    //Pitch wheel values 0 - 8192 - 16384
...
//Unused Pure Virtual Functions
void WavetableOscilatorVoice::controllerMoved(int controllerNumber, int newControllerValue)
{
    juce::Logger::writeToLog("Mod Wheel Value: " + juce::String(newControllerValue));
};
